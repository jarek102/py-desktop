...
# Note: We generate these into ./typings/gi so they land in typings/gi/repository/
for lib in \
    "Astal-4.0" \
    "AstalWp-0.1" \
    "AstalTray-0.1" \
...
do
    echo "  -> Processing $lib..."
    gengir "$lib" -o ./typings/gi > /dev/null
done
...
cp -rf "$SYSTEM_STUBS/"* ./typings/gi/

echo "✅ Done! Monolithic stub library created in ./typings"
